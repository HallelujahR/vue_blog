<template>
	<!-- Modal -->
<div class="modal fade" id="question_comment" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
...
export default{
    	...
    	mounted:function(){
    		...
    		let self = this;
    		axios.get('/question/getComment?tid='+tid)
    	...
    	methods:{
			...
			agree:function(event,id){
	    		//点赞请求
				axios.get('/question/commentAgree?id='+id)
				  .then(function (response){
				  	console.log(response);
				    switch(response['data']){
				    	case 2:
	    	...
	    	commentDel:function(event,id,tid){
				Swal({
				  ...
				  if (result.value) {
				  		axios.get('/question/commentDel?id='+id+'&tid='+tid)
			    		.then(function(response){
			    			console.log(response);
			    			var num = $('#cot').text();
			    			num--;
